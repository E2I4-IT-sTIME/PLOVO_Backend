...
public interface UserRecordRepository extends JpaRepository<UserRecord, Long> {
    ...
    @Query(value = "SELECT u.date, u.weight FROM user_record u WHERE u.user = :user_id and u.date BETWEEN DATE_ADD(NOW(),INTERVAL -6 DAY) AND NOW()", nativeQuery = true)
    ...
    @Query(value = "select MONTH(ur.date) as month, SUM(ur.weight) as weight from user_record ur left join plovo p on p.id = ur.plovo_id\n" +
            "left join mountain m on m.id = p.mountain_id where m.id = 1 and ur.date BETWEEN DATE_ADD(NOW(), INTERVAL -6 MONTH ) AND NOW() GROUP BY MONTH(ur.date);", nativeQuery = true)
    List<MonthAndWeightRes> monthAndWeightRes();
}
